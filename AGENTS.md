# GovArchLab — Project Instructions for OpenAI Codex
...
This file is a Codex-specific pointer and guardrail layer. It does not replace the project operating policy, traceability rules, safe-edit workflow, or governing AI standards.
...
2. [`CLAUDE.md`](./CLAUDE.md) — operational policy for GovArchLab, including the two-file model (tracked vs `-mine`), the required pre-flight checklist, and commit/PR traceability format.
3. [`docs/AI-SESSION-START.md`](./docs/AI-SESSION-START.md) — canonical session bootstrap with current workflow, mount-check requirement, GitHub CLI gotchas, and the drift-prone operating policies.
...
- **Engine edits mirror to the deliverable.** A change to `artifact-engine.html` must be applied surgically to `v2/architecture-overview.html` too — do not run `node build.js` to sync them if a live drawing might be in the file. See CLAUDE.md's "Two-File Model" section.
...
- Confirm the issue has a `Capability:` or `Capability group:` line using the relevant EasyEA capability ID.
...
- `Persona: <persona>`
- What changed, why it changed, and how it was verified (at minimum: mount check passes, no user data was overwritten)
...
Keep each branch and PR focused on the issue being closed. Do not include opportunistic fixes, unrelated refactors, local `-mine` copies, or cleanup that belongs in a separate issue.

Before opening a PR, verify the diff contains only intended files. If unexpected files appear (especially any `v2/*-mine.html` or `v2/.local-*`), stop and remove them from the branch before pushing — those are private working copies and must never be tracked.
